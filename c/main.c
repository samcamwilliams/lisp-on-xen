...
void varbind(long vars, long args);
...
printf("Welcome to lisp-on-xen (adapted from Kernighan & Ritchie Lisp)\n");
printf("Press escape twice to terminate the OS. Have fun!\n");
initialize_atoms();
setbuf(stdin, 0); // Who needs buffers anyway...
setbuf(stdout, 0);
setbuf(stderr, 0);
...
long in_word2(void) { /* read word */
   ...
   while ( buffer == nil ) { /* read in a line */
      ...
      do { /* read characters until '\n' */
         ...
         if (character == 27) {// Kill the VM if the character is escape two times in a row
			 if(getchar() == 27) {
				 printf("\nlisp-on-xen shutting down...\n");
				 exit(0); /* terminate execution */
		     }
			 else
				 getchar();
		 }
		 else if((31 < character && character < 127) || character == 10) {
			 putchar(character);
